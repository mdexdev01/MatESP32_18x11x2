...
void colorWipe(uint32_t color, int wait) {
  for(int i = 0 ; i < 4 ; i++) {
    ...
    for(int j=0; j<strip[i].numPixels(); j++) { // For each pixel in strip[i]...
  ...
  for(int i = 0 ; i < 4 ; i++) {
    for(int a=0; a<10; a++) {  // Repeat 10 times...
      for(int b=0; b<3; b++) { //  'b' counts from 0 to 2...
        strip[i].clear();         //   Set all pixels in RAM to 0 (off)
        // 'c' counts up from 'b' to end of strip in steps of 3...
        for(int c=b; c<strip[i].numPixels(); c += 3) {
    ...
    // for(int a=0; a<30; a++) {  // Repeat 30 times...
      for(int b=0; b<3; b++) { //  'b' counts from 0 to 2...
        ...
        for(int c=b; c<strip[i].numPixels(); c += 3) {
          ...
          int      hue   = firstPixelHue + c * 65536L / strip[i].numPixels() + reminder;
          uint32_t color = strip[i].gamma32(strip[i].ColorHSV(hue)); // hue -> RGB
          strip[i].setPixelColor(c, color); // Set pixel 'c' to value 'color'
        }
        strip[i].show();                // Update strip with new contents
        delay(wait);                 // Pause for a moment
