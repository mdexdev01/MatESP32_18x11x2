...
void configPacket(int num_col, int num_row) {
  ...
  SINGLE_DATA_LEN = 1; // 1: byte, 2: short, 4: int
  PacketRawData = new byte [num_cells * SINGLE_DATA_LEN * 6 + 400 ]; // 400 : in case of overflow
  PacketRawData2 = new byte [num_cells * SINGLE_DATA_LEN * 6 + 400 ]; // 400 : in case of overflow
  PacketData = new byte [num_cells * 2 ]; // 400 : in case of overflow
  PacketData2 = new byte [num_cells * 2 ]; // 400 : in case of overflow

  println("configure packet buffer ...");
}
  ...
  for(int row = 0 ; row < matView_A.numOfRow ; row++ ){ //  ROW = 20
    for(int x = 0 ; x < matView_A.numOfCol ; x++) {   //  COL = 16
      ...
      if(board_index == 0) {
        ...
        matView_B.cellsValue[xy_pos] = byteToInt(read_data[PACKET_HEADER_LEN + xy_pos]);
      }
      else if(board_index == 2) {
        matView_C.cellsValue[xy_pos] = byteToInt(read_data[PACKET_HEADER_LEN + xy_pos]);
      }
