...
void buildOSDPacket(byte [] data, int start_x, int start_y, int osd_width, int osd_height) {
  ...
  data[0] = -1; // 0xFF
  data[1] = -1; // 0xFF
  data[2] = 1; // version major
  data[3] = 0; // version minor
  data[4] = 3; // group id
  data[5] = byte(tx_count % 2); // board id. 0(Right) or 1(Left)
  data[6] = byte(data_len / 100); // data len, over 100
  data[7] = byte(data_len % 100); // data len, under 100
  ...
  data[header_len + data_len] = byte(tx_count % 100);
  data[header_len + data_len + 1] = -2; // 0xFE
