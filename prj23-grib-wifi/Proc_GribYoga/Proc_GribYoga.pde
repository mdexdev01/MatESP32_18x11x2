...
void setup() {
  ...
  //========================================
  //  THREAD - UART
  thread("task_readData_Grib_Assembly");


  //  START TIME STAMP (Optional)
  {
...
void draw_singleTask() {
  boolean result_read = readData_Grib_NoAssembly();
...
boolean task_readData_Grib_Assembly() {
  while(true) {
...
boolean readData_Grib_NoAssembly() {
  if ( isPortOpened == false ) {
    println("port not opened");
    return false;
  }
