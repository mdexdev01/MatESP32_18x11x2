...
//---------------------------------------------
#define DATA_BYTE_LEN  2   //  1: 1byte, 2: 2byte

//  Protocol - Packet
#define HEADER_LEN 8
#define TAIL_LEN 2

#define PACKET_BODY_LEN (NUM_REAL_SENSOR_CH * DATA_BYTE_LEN)
...
int SubBoard_Rx1() {
    ...
    if ((packetBufSlave[IDX_HEADER_0 + jitter] == HEADER_SYNC) && (packetBufSlave[IDX_HEADER_1 + jitter] == HEADER_SYNC)) {
        ...
        if (rx_board_id == 1)
            ...
            tx_grant_board3 = true;
        else if (rx_board_id == 3)
...
int build_count = 0;
void buildPacket_lgd(byte *packet_buffer, int adc_mat_buf[MUX_LIST_LEN][NUM_MUX_OUT], int width, int height) {  // height = 16
    ...
    for (int mux = 0; mux < 8; mux++) {
        for (int y_id = 0; y_id < 16; y_id++) {
            ...
            if (adc_ordered[counter] < 5)
                adc_ordered[counter] = 0;

            if(DATA_BYTE_LEN == 1) {
              packet_buffer[pa_index++] = adc_ordered[counter];
            }
            else if (DATA_BYTE_LEN == 2) { // 0~4096
              packet_buffer[pa_index++] = adc_ordered[counter] / 100;
              packet_buffer[pa_index++] = adc_ordered[counter] % 100;
            }
    ...
    pa_index = HEADER_LEN + PACKET_BODY_LEN;
...
void printPacket(byte *packet_buffer, int packet_len) {
    ...
    else if(single_data_len == 2) {
      for (int i = 0; i < body_len; i++) {
          int data = 0;
          data = packet_buffer[HEADER_LEN + 2 * i] * 100 + packet_buffer[HEADER_LEN + 2 * i + 1];
