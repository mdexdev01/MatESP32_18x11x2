...
class libMatrixView {
  ...
  boolean createView_CSV(String sPathCSV) {
    Table     csvTableRectPos;
    csvTableRectPos = loadTable(sPathCSV, "header");

    if(false == loadCellPos_CSV(csvTableRectPos)) {
  ...
  boolean loadCellPos_CSV(Table table) {
    ...
    try {
      for (TableRow row : table.rows()) {
        ...
        single_data_len = row.getInt("DataByte");
        ...
        println(String.format("\t[libMatrix %s] Column : %d, Row : %d", 
              viewNick, num_col, num_row));
    ...
    println(String.format("\t[libMatrix %s] The EditRect %d cells (garo %d x sero %d) configured.", viewNick, numOfCells, num_col, num_row));
  ...
  void learnGeometry(){
    ...
    println(String.format("\t[libMatrix %s] Outline is (l,r,t,b) : %d, %d, %d, %d", viewNick, left_end, right_end, top_end, bottom_end) );
  }
  ...
  void setEditTextSize(int t_size) {
    textSize = t_size;
    println(String.format("\t[libMatrix %s] setEditTextSize = %d", viewNick, t_size));
  ...
  void setEditTextMargin(int left_margin, int top_margin) {
    println(String.format("\t[libMatrix %s] setEditTextMargin = %d", viewNick, left_margin, top_margin));
    
    for(int cell_index = 0 ; cell_index < numOfCells ; cell_index++){
      cellsRect[cell_index].setTextLeftMargin(left_margin);
      cellsRect[cell_index].setTextTopMargin(top_margin);
    }
  ...
  void drawColorCells(){  
    ...
    for ( i = 0 ; i < numOfCells ; i++ ) {
      ...
      if(cellsValue[i] == 0) {
        ...
        else if(singleDataLen == 2) {
          fill(colorMap(cellsValue[i] / 8), 90, 90); // HUE, SATURATE, BRIGHTNESS : black(0, 0, 0) . white(0, 0, 100), color (180~0, 100, 100)
        }
      }
      cellsRect[i].render();  
  ...
  void fillPacket2B(byte [] origin_data, int offset) {
    for(int i = 0 ; i < numOfCells ; i++) {
      int data = byteToInt(origin_data[offset + i * 2]) * 100 + byteToInt(origin_data[offset + i * 2 + 1]);
      cellsValue[i] = data;
    }
  }
