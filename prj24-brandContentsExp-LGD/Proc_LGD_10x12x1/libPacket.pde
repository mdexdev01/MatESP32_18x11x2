...
static final int SINGLE_DATA_LEN     = 2; // 1: byte, 2: short, 4: int
...
void configPacket(int num_col, int num_row) {
  ...
  PACKET_LEN_TYPE0     = PACKET_HEADER_LEN + PACKET_TAIL_LEN + packetCells * SINGLE_DATA_LEN;
...
int parseData_LGD(byte[] read_data) {
  int parse_error = 0;
  ...
  matView_A[board_id].fillPacket2B(read_data, PACKET_HEADER_LEN);
